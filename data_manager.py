...
def update_answer(cursor, message, image_url, id):
    cursor.execute("""
                   UPDATE answer
                   SET message= %(message)s, image= %(image_url)s
                   WHERE id=%(id)s
                   """,
                   {'message': message, 'image_url': image_url, 'id': id})
...
def get_sorted_questions(cursor, column_to_order_by, asc=True, limit=None):
    ...
    command = f"""
               SELECT question.id, submission_time, user_account.name
               AS posted_by, view_number, vote_number, title, message, image
               FROM question
               FULL JOIN user_account ON user_id = user_account.id
